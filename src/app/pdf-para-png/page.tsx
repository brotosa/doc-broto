...
export default function Page() {
  ...
  const run = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= doc.numPages; i++) {
        ...
        zip.file(`pagina-${i}.png`, blob);
      }

      if (doc.numPages === 1) {
        const only = await zip.file("pagina-1.png")!.async("blob");
        downloadBlob(only, "pagina-1.png", "image/png");
      } else {
        const out = await zip.generateAsync({ type: "blob" });
        downloadBlob(out, "imagens-png.zip", "application/zip");
      }
    } catch (e) {
  ...
  return (
    <ToolShell tool={tool}>
      ...
      >
        {busy ? "Convertendo..." : "Converter para PNG"}
      </button>
    </ToolShell>
  );
}
